...
class XLAHLmTrainer(BaseXLATrainer):
    ...
    def combined_clip_perc(self, mask, clip_mask, kl_clip, acc_clip):
        return 1 - (1-(kl_clip | acc_clip).float()) * (clip_mask.float().sum() / mask.float().sum())
    ...
    def train_step(self, step, model, x, mask):
        ...
        results = DotDict(
            ...
            combined_clip_perc=self.combined_clip_perc(mask, clip_mask, kl_clip, acc_clip),
        ...
        results.one_minus_combined_clip_perc = 1 - results.combined_clip_perc
