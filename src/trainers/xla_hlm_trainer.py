...
class XLAHLmTrainer(BaseXLATrainer):
    ...
    def loss(self, token_loss, kl_loss, uncond_kl_loss, kl_clip):
    ...
    def train_step(self, step, model, x, mask):
        ...
        results = DotDict(
            ...
            kl_per_token=self.kl_per_token(kl, cond_mask),
            kl_per_token_nopad=self.kl_per_token_nopad(kl, cond_mask, cond_x, model.config.pad_token_id),
        ...
        results.loss = self.loss(results.token_loss, results.kl_loss, results.uncond_kl_loss, kl_clip)
