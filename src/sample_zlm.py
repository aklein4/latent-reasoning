...
CHECKPOINT = 'proto-zlm_beta/000000006000/checkpoint.ckpt'
...
def main():
    ...
    output = model.sample(tokens, temperature=2.0)
    full = torch.cat((tokens, output), dim=-1)

    print(f"\nOUTPUT:\n{tokenizer.decode(full[0])}")
